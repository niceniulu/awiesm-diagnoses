...
fname = 'sss.fesom.orig.yearmean.nc'
...
for i in range(0, nyear, dyear):
    ...
    # iceyear = i*20 - 32000
    theyear = years[i]
    ax.set_title('SSS year = '+str(theyear), fontsize=15)
    ...
    cbar.set_label(label='SSS', size=13, ) #weight='bold')
...
for i in range(0, nyear, dyear):
    ...
    # iceyear = i*20 - 32000
    theyear = years[i]
    ax.set_title('SSS anomaly  year = '+str(theyear), fontsize=15)
    ax.set_xlabel("Longitude (°)", fontsize=15)
    ax.set_ylabel("Latitude (°)", fontsize=15)
    ...
    cax = plt.axes([0.1,0.05,0.85,0.03])
    cbar = plt.colorbar(cs,cax=cax, orientation='horizontal', fraction=0.02, extend='both')
    cbar.set_label(label='SSS anomaly', size=13, ) #weight='bold')
    cbar.ax.tick_params(labelsize=15)  #labelsize=15
    # cbar.set_ticks(np.arange(minval,maxval+0.1,0.25))
