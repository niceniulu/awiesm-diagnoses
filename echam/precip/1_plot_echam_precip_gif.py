...
fname = 'tran-17k-hosing_echam_precip_summer.nc'
...
data = ds['aprl'].values + ds['aprc'].values
data = data*3600*24*365./1000.   # kg/m2s to m/year
...
color = plt.cm.YlGnBu  #cmocean.cm.thermal   #haline
...
for i in range(0, nyear, dyear):
    ...
    cbar.set_label(label='precip', size=13, ) #weight='bold')
...
nbreaks = 51
breaks = np.linspace(minval ,maxval ,nbreaks)
color = plt.cm.BrBG   #cmocean.cm.phase  #haline
...
for i in range(0, nyear, dyear):
    ...
    # iceyear = i*20 - 32000
    theyear = years[i]
    ax.set_title('precip anomaly  year = '+str(theyear), fontsize=15)
    ax.set_xlabel("Longitude (°)", fontsize=15)
    ax.set_ylabel("Latitude (°)", fontsize=15)
    ...
    cax = plt.axes([0.1,0.05,0.85,0.03])
    cbar = plt.colorbar(cs,cax=cax, orientation='horizontal', fraction=0.02, extend='both')
    cbar.set_label(label='precip anomaly', size=13, ) #weight='bold')
    cbar.ax.tick_params(labelsize=15)  #labelsize=15
    # cbar.set_ticks(np.arange(minval,maxval+0.1,0.25))
...
gifname = "animated_precip_anomaly.gif"                    ############## define !!!
# Save to GIF
imageio.mimsave(gifname, frames, duration=0.3)  # duration in seconds per frame
